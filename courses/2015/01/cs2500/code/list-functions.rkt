...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname list-functions) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ())))
...
(define FRUIT (list "Apple" "Pear" "Banana"))
...
(check-expect (in? (list "One" "Two") "Three") false)
(check-expect (in? (list "One" "Two") "Two") true)
...
; Number Number -> LON
(check-expect (gen-range 0 5) (list 0 1 2 3 4))

(define (gen-range x y)
  (cond
    [(>= x y) empty]
    [else (cons x (gen-range (+ x 1) y))]))