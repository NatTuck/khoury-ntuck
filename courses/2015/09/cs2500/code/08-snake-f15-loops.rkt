...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname 07-snake-f150-loops) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
...
(define (got-key w ke)
  (if (string=? ke " ")
      (make-world (jump-snake (world-snake w))
                  (world-food w))
...
; Snake -> Snake
; Make the snake jump up by one.
(define (jump-snake s)
  (make-snake (snake-dir s)
              (map jump-seg (snake-segs s))))
...
(define (hit-any? s los)
  (local (; Seg Seg -> Boolean
          (define (seg-pos=s? s1)
            (posn=? (seg->posn s) (seg->posn s1))))
    (ormap seg-pos=s? los)))
...
(define (draw-snake s scn)
  (foldr draw-seg scn (snake-segs s)))
