...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname snake-final-130) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
...
; A SnakeWorld is (make-sw Posn Snake Number Number)
...
; note: When we get this from on-key, it may have other
;   values. Include "else" in template.
#;
(define (dir-tmpl dir)
...
; SnakeWorld -> Boolean
; Has the snake crashed into anything, ending the game?
(define (game-over? sw)
  (snake-dead? (sw-snake sw)))

; Snake -> Boolean
; Is the snake dead?
(define (snake-dead? sn)
  (or (snake-hit-wall? sn)
      (snake-hit-self? sn)))

; Snake -> Boolean
; Does the head of the snake overlap with rest of snake?
(define (snake-hit-self? sn)
  (seg-overlap? (first (snake-segs sn))
                (rest  (snake-segs sn))))

; Seg LoSegs -> Boolean
; Does the head hit any others?
(define (seg-overlap? head body)
  (cond [(empty? body) false]
        [(cons? body) (or (posn=? head (first body))
                          (seg-overlap? head (rest body)))]))
 
; Snake -> Boolean
; Did the snake hit a wall?
(define (snake-hit-wall? sn)
  (head-hit-wall? (first (snake-segs sn))))

; Seg -> Boolean
; Is this segment out of bounds?
(define (head-hit-wall? seg)
  (or (< (posn-y seg) 0)
      (>= (posn-y seg) GRID-HEIGHT)
      (< (posn-x seg) 0)
      (>= (posn-x seg) GRID-WIDTH)))

; SnakeWorld Dir -> SnakeWorld
; Change direction based on which arrow key user presses.
(define (change-dir sw k)
   (if (or (string=? k "up")
           ...
           (string=? k "right"))
       (make-sw (sw-food sw)
                (steer-snake (sw-snake sw) k)
                (sw-delay sw))
       sw))

; Snake Dir -> Snake
; Steer the snake.
(define (steer-snake sn d)
  (make-snake d
              (snake-segs sn)))
...
(define (update-world sw)
  (if (< (sw-delay sw) 1)
      (make-sw (update-food (sw-food sw) (sw-snake sw))
               ...
               (calc-new-delay (sw-snake sw)))
      (make-sw (sw-food sw)
               (sw-snake sw)
               (sub1 (sw-delay sw)))))

; Snake -> Number
(define (calc-new-delay sn)
  (- 11 (length (snake-segs sn))))
...
; LoSegs -> LoSegs
; Remove the last segment in a list of segs.
