...
public class DivisibleLengths {
  ...
  public static class TokenizerMapper 
    ...
    private IntWritable prime = new IntWritable();
      ...
      while (itr.hasMoreTokens()) {
        ...
        if (!ww.equals("")) {
          for (int i = 1; i <= ww.length(); i++) {
            if (ww.length() % i == 0 && isPrime(i)) {
              prime.set(i);
              context.write(prime, one);
            }
          }
        }
      }
  ...
  public static class PrimeReducer extends Reducer<IntWritable, IntWritable, IntWritable, IntWritable> {
